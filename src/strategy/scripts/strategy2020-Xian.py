...
class Strategy(NodeHandle):
    ...
    def Avoidance_Strategy(self):  # Waiting for modification
        ...
        #===========mid = self._scan[0]
        #===========
        for i in range(len(self._scan)-5):
            delta = i
            if(delta>180):
                delta-=360#mid:0 +-180
            if(abs(delta)<10):
                beta=(abs(delta)+70)/100.0#+70%
                if(beta>1):
                    beta=1
                self._scan[i]*=beta#close mid rotate quick
        #===========
        #===========mid have obstacle -> bigger right left
        check_angle = 0
        check_sum = 0
        add_angle = 10
        for i in range(len(self._scan)-5):
            ...
            if(delta>180):
                delta-=360#mid:0 +-180
            if(abs(delta)<20):
               check_sum += self._scan[i]*np.sign(delta)
        if(check_sum>0):#obstacle is left
            check_angle -=add_angle
        else:
            check_angle +=add_angle
        #===========
        for i in range(len(self._scan)-5):  # -10 debug
            # left and right too close
            mid_check_ang = 20
            if((i < mid_check_ang or i > (360-mid_check_ang)) and self._scan[i] < 0.75):
                have_obstale = True
            ...
            if(self._scan[i] < avoidance_distance):
                ...
                if(i < avoidance_angle+check_angle):  # left side
                    obstacle_num += 1
                    obstacle -= (avoidance_distance - self._scan[i])
                if(i > (360+check_angle-avoidance_angle)):  # right side
                    obstacle_num += 1
                    obstacle += (avoidance_distance - self._scan[i])
        if(obstacle_num):
    ...
    def Find_Ball_Strategy2(self):
        if(self.state == 0):
            ...
            RPang = Norm_Angle((self.angleFor_findBall-self.findang) - self._front)
            # if(self.lostball == True):
            #     RPang = Norm_Angle((self.ballang-self.findang) - self._front)
            # print("f-f  {}  F  {}  G {}".format(self.angleFor_findBall-self.findang,self._front,RPang))
            ...
            if(abs(RPang) > self.error_ang):
                ...
                self.Robot_Vel([0, z])
                if(self.lostball == True):
                    if(self._ballsColor == self.ballcolor and self._ballsDis < (self.balldis+50)):
                        self.ballcolor = self._ballsColor
                        self.balldis = self._ballsDis
                        self.ballang = self._front
                        print('find ball')
                        self.Robot_Stop()
                        # print("catch ball: ", self.color_to_strings(self.ballcolor))
                        self.behavior = CATCH_BALL
                        self.state = 0
                        self.lostball = False
                else:
    ...
    def Catch_Ball_Strategy(self):  # chen
        self.lostball = False
	#print("catch state",self.state)
        if(self.state == 0):#rotate to the stored ball angle
            if(self.ballang == 999):
                self.Robot_Stop()
                self.behavior = FIND_BALL
                self.state = 0
                ...
                if(abs(RBang) > 2):
                    ...
                    self.Robot_Stop()
                    self.state = 1
        elif(self.state == 1):#check to the see ball angle
            ...
            if(color != None):
                ...
                self.ballang = self._front + RBang/5
                ...
                self.ballang = 999
                self.ballarea = 0
        elif(self.state == 2):#go to the ball position
            color = None
            ...
            if(color != None):
                ...
                if(RPdis > self.catchBallDis):
                    x = self.vel_x if(RPdis > 150) else 0.1#self.slow_vel_x
                    if(abs(RBang) > self.error_ang):
                        z = self.z_speed_planning(RBang)
                        #z = self.vel_z if(RBang > self.error_ang) else -self.vel_z
                        # z = 0.2 if(RBang > self.error_ang) else -0.2
                    else:
                        z = 0
                    #if(self.prev_RPdis >= RPdis):
                    ...
                    self.Robot_Vel([x, z])
                    self.prev_RPdis = RPdis
                    #print("rpdis x", RPdis, x)
                    #print("RBang", RBang/10.0)
                else:
                    x = 0.05
                    self.Catch_Ball(1)
                    switcher = {
